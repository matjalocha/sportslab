"""Tests for PlattScaler calibration."""
...
class TestPlattScalerBinary:
    """Tests for binary classification Platt scaling."""
    ...
    def test_perfect_calibration_stays_close(
        ...
        assert np.allclose(calibrated, probs, atol=0.15)
...
class TestPlattScalerMulticlass:
    """Tests for multiclass (1X2) Platt scaling."""
...
class TestPlattScalerEdgeCases:
    """Edge case tests for PlattScaler."""
    ...
    def test_single_class_binary(self) -> None:
        ...
        scaler = PlattScaler()
        # Platt needs both classes, give it a mix
        labels[:5] = 0
        probs[:5] = 0.1
        calibrated = scaler.fit_transform(labels, probs)
    ...
    def test_small_sample(self) -> None:
        """Should work with few samples (minimum viable for LR)."""
        labels = np.array([0, 1, 1, 0, 1, 0, 1, 0])
        probs = np.array([0.3, 0.8, 0.7, 0.2, 0.9, 0.1, 0.6, 0.4])
        scaler = PlattScaler()
        calibrated = scaler.fit_transform(labels, probs)
